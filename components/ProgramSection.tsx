...
/**
 * ProgramSection 컴포넌트
 * K Book WOW의 5대 핵심 프로그램(온라인 리딩, 그림책 창작, 창의적 글쓰기, 한국 문화 공예, 도예 융합)을
 * 시각적인 카드 그리드와 인터랙티브 상세 모달 형태로 제공합니다.
 */
  ...
  const programs: Program[] = [
    ...
    },
    {
      ...
      details: {
        ...
        outcomes: [
          "촉각과 입체 조형을 결합한 몰입도 높은 언어 체득",
          "흙의 성질과 한국 전통 도자기(청자/백자/옹기) 이해",
          "실제 실생활에서 쓸 수 있는 나만의 도자기 식기/오브제 제작",
        ],
        curriculum:
          "흙 반죽(점토 탐색) → 도예 기법(핀칭, 코일링, 물레) 실습 → 우리말 각인 및 가마 소성",
      },
    },
  ];

  return (
    <section id="programs" className="relative py-28 bg-[#090A10]">
      {/* 장식용 은은한 배경 광원 */}
      <div className="absolute top-1/2 left-0 w-96 h-96 bg-indigo-600/10 blur-[130px] pointer-events-none" />
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-pink-600/10 blur-[130px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {programs.map((prog) => (
            <div
              key={prog.id}
              className={`group relative rounded-3xl overflow-hidden glass-card border transition-all duration-300 flex flex-col ${prog.accentColor}`}
            >
              {/* 카드 상단 이미지 영역 */}
              <div className="relative aspect-[16/10] w-full overflow-hidden bg-slate-900">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-[#0E111B] via-black/20 to-transparent" />

                {/* 뱃지 */}
                <div className="absolute top-4 left-4">
                  <span
                    className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold text-white bg-gradient-to-r ${prog.badgeColor} shadow-md`}
                  >
                    {prog.badge}
                  </span>
                </div>
              ...
              <div className="p-6 sm:p-7 flex-1 flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-2 text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2 font-['Outfit']">
                    {prog.icon}
                ...
                <div className="pt-4 border-t border-white/10 flex items-center justify-between">
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 text-xs font-bold text-indigo-400 hover:text-pink-400 transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="px-3.5 py-1.5 rounded-full text-xs font-semibold bg-white/10 hover:bg-white/20 text-white transition-colors"
                  >
                    수업 신청
                  </button>
                </div>
      ...
      {selectedProgram && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in-up">
          <div className="relative w-full max-w-2xl bg-[#0F121C] border border-white/20 rounded-3xl overflow-hidden shadow-2xl max-h-[90vh] flex flex-col">
            {/* 닫기 버튼 */}
            <button
            ...
            <div className="p-6 sm:p-8 overflow-y-auto space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="p-4 rounded-2xl bg-white/5 border border-white/5">
                  <div className="text-xs font-bold text-pink-300 mb-1">
                    수업 방식
                  </div>
                  <div className="text-sm font-medium text-slate-200">
              ...
              <div>
                ...
                <div className="p-4 rounded-2xl bg-indigo-950/40 border border-indigo-500/20 text-sm text-indigo-200">
              ...
              <div className="pt-4 flex items-center justify-end gap-3">
                <button
                  onClick={() => setSelectedProgram(null)}
                  className="px-5 py-2.5 rounded-full text-sm font-medium text-slate-400 hover:text-white"
                >
                  닫기
                </button>
                <button
                  ...
                  className="px-6 py-2.5 rounded-full text-sm font-bold text-white bg-gradient-to-r from-indigo-600 to-pink-500 shadow-lg hover:scale-105 transition-all"
                >
                  이 프로그램 상담 신청하기
                </button>
              </div>
